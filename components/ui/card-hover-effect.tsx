...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-3 w-full h-full grid-rows-[1fr_1fr]",
        className
      )}
    >
      {items.map((item, idx) => (
...
export const CardTitle = ({
  ...
  return (
    <h4 className={cn("text-black font-bold tracking-wide lg:text-2xl font-integral sm:text-5xl", className)}>
      {children}
    </h4>
  );
};
export const CardDescription = ({
  ...
  return (
    <p
      className={cn(
        "mt-2 text-black tracking-wide leading-relaxed lg:text-sm font-integral z-0 sm:text-2xl",
        className
      )}
    >
      {children}
